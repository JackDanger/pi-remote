...
import { parseClientRequest, ProtocolError, } from "./protocol.js";
const CONTENT_TYPES = {
...
export function cacheControlFor(filePath) {
    ...
    if (CACHEABLE_MEDIA_EXTENSIONS.has(path.extname(filePath)))
...
function toImageContents(images) {
    return images?.map((image) => ({ type: "image", data: image.data, mimeType: image.mimeType }));
}
async function dispatch(request, options, client, sessionsChanged) {
    ...
    switch (request.type) {
        ...
        case "session.compact":
            ...
            return {};
        case "session.compact_abort":
            sessionHost.abortCompaction(request.sessionId);
            return {};
