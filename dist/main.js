...
const telemetry = config.telemetry
    ? new Telemetry({
        ...
        onSnapshot: (sessionId, snapshot) => {
            const push = { type: "session_telemetry", sessionId, telemetry: snapshot };
            sessionHost.pushToAttached(sessionId, push);
        },
    })
    : undefined;
const sessionHost = new SessionHost(environment.hostDeps, telemetry);
...
server.on("listening", () => {
    ...
    if (config.host !== "127.0.0.1" && config.host !== "localhost" && config.host !== "::1") {
        console.warn(`WARNING: bound to ${config.host} — pi-remote has NO authentication; anyone who can reach this port can run arbitrary commands as this user. Front it with an authenticating reverse proxy.`);
    }
});
...
async function shutdown(signal) {
    ...
    console.log(`${signal} received — draining ${streaming.length} running turn(s), grace ${config.shutdownGraceMs}ms (send ${signal} again to force exit)`);
    server.close();
